...
def next(comando):
    global pos_mus
    global playlist
    if comando[1] in playlist[:pos_mus:]:
        comando_index = playlist.index(comando[1])
    else:
...
while True:
    ...
    if comando[0] == 'play' and not tocando and playlist != []:
        ...
        if comando[1] in playlist:
            ...
            #if tocando and len(playlist) > 1:
                #if pos_mus == (len(playlist) - 1) and comando[1] != playlist[pos_mus] and comando[1] != playlist[0]:
                    #next(comando)
            elif tocando and (comando[1] in playlist[:pos_mus:] or comando[1] in playlist[pos_mus+2::]):
                next(comando)
